...
if [ ${stage} -le 1 ]; then
    ...
    steps/make_mfcc_pitch_online.sh --cmd "$train_cmd" \
                                    ...
                                    data/all_${lang} exp/make_mfcc_pitch_online/train_${lang} ${mfccdir}

    ./steps/compute_cmvn_stats.sh data/all_${lang}
...
if [ ${stage} -le 2 ]; then
    ...
    # make json labels
    mkjson.py ${feat_tr_dir}/feats.scp data/tr_${lang} ${dict} > ${feat_tr_dir}/data.json 
    mkjson.py ${feat_dt_dir}/feats.scp data/dt_${lang} ${dict} > ${feat_dt_dir}/data.json 

    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        mkjson.py ${feat_recog_dir}/feats.scp data/${rtask} ${dict} > ${feat_recog_dir}/data.json   
    done
fi
